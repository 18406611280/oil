...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.save = function (successCallback) {
        ...
        svgDOM = svgDOM.split(/marker-start="url\("#/g).join("marker-start=\"url(#").split(/start"\)"/g).join("start)\"");

        // svgDOM = svgDOM.replace(/marker-mid="url\("#/g,"marker-mid=\"url(#").replace(/mid"\)"/g,"mid)\"");
        svgDOM = svgDOM.split(/marker-mid="url\("#/g).join("marker-mid=\"url(#").split(/mid"\)"/g).join("mid)\"");

        // svgDOM = svgDOM.replace(/marker-end="url\("#/g,"marker-end=\"url(#").replace(/end"\)"/g,"end)\"");
        svgDOM = svgDOM.split(/marker-end="url\("#/g).join("marker-end=\"url(#").split(/end"\)"/g).join("end)\"");
